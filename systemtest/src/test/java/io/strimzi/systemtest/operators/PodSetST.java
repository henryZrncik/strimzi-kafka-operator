...
public class PodSetST extends AbstractST {
    ...
    void testPodSetOnlyReconciliation() {
        ...
        resourceManager.createResourceWithWait(KafkaTemplates.kafkaPersistent(testStorage.getNamespaceName(), testStorage.getClusterName(), replicas)
            .editSpec()
                .editOrNewKafka()
        ...
        DeploymentResource.replaceDeployment(clusterOperator.getDeploymentNamespace(), clusterOperator.getClusterOperatorName(),
            coDep -> coDep.getSpec().getTemplate().getSpec().getContainers().get(0).setEnv(envVars));
